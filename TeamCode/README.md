# Introduction
This is the TeamCode module for the FTC Infinity Tech #13684 robotics team.  This README documents the Infinity Tech team software tool setup and architecture for the 2020-2021 Ultimate Goal season here.

# Installing Android Studio
Our team uses Android Studio for all development.  Here are the basic instructions for installing Android Studio and setting it up:
1.  Download Android Studio.  Unfortunately the latest version (4.1) has broken the plugin we use for wireless development over Wifi, so you'll need to download the older 4.0.1 version from one of the following links:  
...
Chromebook: Sadly, Android Studio is not allowed on school issued chromebooks, but if you have your own, please [shoot me an email](mailto:kwrush024@pccsk12.com).
...
4.  If it isn't already selected, in the top bar, select "Marketplace". Type in "Android Wifi ADB" plugin in the left search box, select the first one in the list, and click the "Install" button on the right (note that the plugin is already installed on my computer):
...
2.  Enter the team repository URL below into the "Get From Version Control" dialog:  https://github.com/scottarush/InfinityTech_UltimateGoal:  For the "Directory:" field, enter a location on your computer's hard drive to store your copy, such as _C:\Documents\My_UltimateGoal_Code_, but you can enter any directory you want:
![Enter Github URL](docs/entering_github_url_in_android_studio.PNG)
3.  Click the "Clone" button at the bottom, and Android Studio should automatically clone the team repository into your selected directory.  Once the download finishes, Android Studio will build the code. If you have issues, we'll need discuss those at the next robotics meeting. 

### Creating a Github Account
Our InfinityTech_UltimateGoal github repository is public, so you can clone it without requiring github credentials.  However, if you want to make changes to the code, you'll need to make one. If you don't already have an account, then go to https://github.com/join create your account.
Send an email with your account username to [me](mailto:kwrush024@pccsk12.com) so that we can add you to the collaborators list to be able to make code changes.  

## Rev Control and Expansion Hubs
...
### Developing with the Rev Control Hub
Starting with the 2020-2021 Ultimate Goal season, InfinityTech uses Control hubs and, if we need more outputs, an attached Expansion hub.  We don't use an Expansion hub and a "Robot Controller" mobile phone on the robot.  Developing robot code using the Control hub is much much faster, and doesn't wear out the connectors.  The basic process is:
...
4.  Connect your computer to the Control Hub's Wifi Hotspot.  We named _InfinityTech13684_.  Email Kyle, and he can give you the password.
5.  Open a Windows or Mac command shell and connect the "Android Debug Bridge", or _adb_, to the robot with the following command:  _adb connect 192.168.43.1_.
This lets Android Studio know about the connected robot (through adb). If there are any errors, we will discuss them in the following robotics meeting.
...
~~~
    Start
        Entry {
            rotateToHeading(90); // Rotate 90 Degrees
        }
    {
    ...
    Drive
        Entry{
            moveStraight(24d); // Move foward 20 inches (d = double floating point number)
         }
    {
    ...
    RotateBack
        Entry{
            rotateToHeading(0); // Rotate back 90 degrees
        }
    {
    ...
    StrafeBack
        Entry{
            strafe(-24d); // Strafe back 24 inches
        }
    {
...
The gen_autosm.bat file  points to a _Smc.jar_ file located in the _TeamCode/lib_ directory.  Note that _Smc.jar_ is only used to re-generate the state machine Java code.  _Smc.jar_ isn't used as a library at runtime. Only 4 base class files are needed at runtime, and these have been copied into the _Teamcode/statemap_ package.
...
Infinity Tech implements these two functions using a _Basic Kalman Filter_ and a _PID_ controller framework called the _Guidance Controller_.  This section describes both of these functions.  The software for both is in the _org.firstinspires.ftc.teamcode.guidance_ package.
...
Kalman Filters are now used widely in almost all navigation systems in robots, autonomous vehicles, etc..  Infinity Tech's KalmanFilter code is in the `org.firstinspires.ftc.teamcode.guidance` package, and it allows us to combine measurements from the wheel motor encoders and the IMU to determine the robot's position and heading during autonomous mode.  We then use the estimated position and heading to feed the GuidanceController described below and our State Machine Compiler scripts.
